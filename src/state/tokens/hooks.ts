...
import { addPoolAddresses, addTokenKeys, updateChartData, updatePriceData, updateTokenData, updateTransactions } from './actions'
 ...
 * @param timeWindow
 */
export function useTokenPriceData(address: string, interval: number, timeWindow: OpUnitType): PriceChartEntry[] | undefined {
  const dispatch = useDispatch<AppDispatch>()
  const [activeNetwork] = useActiveNetworkVersion()
  const token = useSelector((state: AppState) => state.tokens.byAddress[activeNetwork.id]?.[address])
  ...
  useEffect(() => {
    async function fetch() {
      const { data, error: fetchingError } = await fetchTokenPriceData(address, interval, startTimestamp, dataClient, blockClient)
      if (data) {
        dispatch(
          updatePriceData({
  ...
  }, [activeNetwork.id, address, blockClient, dataClient, dispatch, error, interval, oldestTimestampFetched, priceData, startTimestamp, timeWindow])
